...
- Selected relation in Free Sandbox can be reset with a single action
- Session persistence utilities (`Save Session`, `Restore Session`, `Clear Session`) backed by `sessionStorage`
- Additional deduction board for manual cross-out marks (`✓/✗`) with partial auto-fill from glued/same-house links
- Non-interactive transparent clues panel on the right side with Polish clue set text
